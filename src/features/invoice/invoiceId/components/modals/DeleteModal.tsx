...
export default function DeletModal() {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onError: (error) => {
      toast.error(parseError(error) || 'Error deleting Payment!');
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({
        predicate: (query) => query.queryKey[0] == INVOICE.CREATE
      });
      toast.success(data.data.message || 'Payment deleted successfully!');
      toggleModals({});
    }
  });

  return (
    <XModal isOpen={isOpen.DELETE_PAYMENT_MODAL} closeModal={() => isPending || toggleModals({})}>
